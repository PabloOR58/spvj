...
if st.session_state.selected_game:
    ...
    with col_a:
        st.markdown(f'<img src="{get_game_background(appid)}" onerror="this.src=\'{IMG_ERROR}\';" style="width:100%; border-radius:10px; min-height:200px; object-fit:cover;">', unsafe_allow_html=True)
...
st.title("🎮 infosteam")
if df_listado.empty: 
    st.warning("No se encontraron datos en la carpeta Clean.")
    st.stop()
...
m3.metric("🏆 King of the Day", fix_nan(df_day.iloc[0]["Nombre"]) if not df_day.empty else "-")
st.divider()
    ...
    for r in range(0, len(top_df), 5):
        ...
        for i, col in enumerate(cols):
            ...
            if idx < len(top_df):
                ...
                with col:
                    # Contenedor con fallback HTML
                    st.markdown(
                        f"""
                        <div style="height: 140px; overflow: hidden; border-radius: 5px; background: #161921; display: flex; align-items: center; justify-content: center;">
